...
- Runs multiple independent AirPlay receiver slots simultaneously (1-100), each advertised as a separate Bonjour service on your local network (e.g., `AirCapture-01`, `AirCapture-02`, ...)
- For best performance with many streams, use a Mac with sufficient RAM and a wired Ethernet connection
...
Distribution scripts are kept locally and are not part of the public repository. To build a release binary, use `xcodebuild` directly:

```bash
cd AirCapture
xcodebuild -project AirCapture.xcodeproj -scheme AirCapture -configuration Release \
    CODE_SIGN_IDENTITY="Developer ID Application" \
    DEVELOPMENT_TEAM=YOURTEAMID \
    clean build
```

Output: `~/Library/Developer/Xcode/DerivedData/.../Build/Products/Release/AirCapture.app`
...
├── scripts/                        Distribution scripts (local only, not in repo)
...
## Building the vendor library

`vendor/uxplay/build_lib.sh` builds the UxPlay C library from source using CMake. Run once after cloning, or whenever vendor sources change.
...
`AirCapture/Scripts/embed_libraries.sh` runs automatically as a post-compile Xcode build phase. It copies OpenSSL and libplist dylibs from Homebrew into the app bundle's `Contents/Frameworks/`, rewrites install names to `@rpath`, and re-signs them.
